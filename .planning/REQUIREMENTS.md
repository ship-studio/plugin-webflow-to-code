...
- [ ] **ZIP-01**: User can select a Webflow export .zip via file picker in the plugin modal
- [ ] **ZIP-02**: Plugin extracts zip contents to a temp directory via shell.exec unzip
- [ ] **ZIP-03**: Plugin validates zip structure (checks for index.html, CSS files) and shows clear error for malformed exports
...
- [ ] **ASST-02**: Brief contains an asset manifest table listing every copied asset with path, inferred purpose, and referencing page(s)
- [ ] **ASST-03**: Plugin groups responsive image variants (-p-500, -p-800, etc.) as srcset families in the asset manifest
...
- [ ] **BREF-02**: Brief contains mode-specific behavioral instructions (Pixel Perfect: exact dimensions, fixed units; Best Site: semantic HTML, responsive patterns)
- [ ] **BREF-03**: Brief includes a multi-session migration scaffold (ordered page list, progress tracking format, resume instructions)
- [ ] **BREF-04**: Plugin shows approximate token count in the results UI after brief generation
